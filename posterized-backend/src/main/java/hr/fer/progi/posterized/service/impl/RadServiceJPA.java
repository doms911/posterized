...
public class RadServiceJPA implements RadService {
    ...
    public void updateRad(String admin, String stariNazivRad, String nazivRad, String ime, String prezime, String email, MultipartFile poster, MultipartFile pptx) {
        ...
        if(!(nazivRad == null) && !nazivRad.equalsIgnoreCase(stariNazivRad)) rad.setNaslov(nazivRad);
        if(!(ime == null) && !ime.equalsIgnoreCase(rad.getAutor().getIme())) rad.getAutor().setIme(ime);
        if(!(prezime == null) && !prezime.equalsIgnoreCase(rad.getAutor().getPrezime())) rad.getAutor().setPrezime(prezime);
        if(!(email == null) && !email.equalsIgnoreCase(rad.getAutor().getEmail())) rad.getAutor().setEmail(email);
        if(!poster.isEmpty()) {
            Media objekt = new Media();
