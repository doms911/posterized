\chapter{Arhitektura i dizajn sustava}
	...
	\end{itemize}
			...
			Entitet \textbf {Rad} sadrži sve važne informacije o radu. Sadrži atribute: ID rada, naziv postera, naziv prezentacije i naslov rada. Atribut naziv prezentacije je opcionalan te stoga može poprimiti vrijednost null. Entitet Rad u ternarnoj je vezi s entitetima Autor i Konferencija \textit{(One-to-One-to-Many)}.
				\begin{longtblr}[
					label=none,
					entry=none
					]{
			...
			\noindent Entitet \textbf {Autor} sadrži informacije o autoru. Sadrži atribute: OIB autora, email autora i ime i prezime autora. Entitet Autor u ternarnoj je vezi s entitetima Rad i Konferencija \textit{(One-to-One-to-Many)}.
				\begin{longtblr}[
					label=none,
					entry=none
					]{
			...
			\noindent Entitet \textbf {Konferencija} sadrži informacije o stručnoj konferenciji koja će se održati. Sadrži atribute: ID konferencije, poveznica na video prijenos konferencije, pin za ulazak na konferenciju, vrijeme početka i vrijeme kraja konferencije, ID admina zaduženog za konferenciju i poštanski broj mjesta u kojem se održava konferencija. Entitet Konferencija u ternarnoj je vezi s entitetima Rad i Autor \textit{(Many-to-One-to-One)}, u binarnoj vezi \textit{(Many-to-Many)} s entitetom Korisnik, u vezi \textit{(Many-to-One)} s entitetom Mjesto, \textit{(One-to-Many)} s entitetom Fotografija, \textit{(Many-to-One)} s entitetom Admin i \textit{(Many-to-Many)} s entitetom Pokrovitelj.
				\begin{longtblr}[
					label=none,
					entry=none
					]{
...
trenutnih događanja u glavnoj konferencijskoj dvorani\\ \hline 
			...
			\noindent Entitet \textbf {Prijava} sadrži informacije o autoru, radu koji je prijavio na određenu konferenciju te broju glasova koji je osvojio na toj konferenciji za svoj rad. Sadrži atribute: ID rada, ID konferencije, ukupan broj osvojenih glasova i OIB autora. Entitet Prijava rezultat je ternarne veze entiteta Autor, Rad i Konferencija.
				\begin{longtblr}[
					label=none,
					entry=none
					]{
					...
					\hline \SetCell[c=3]{c}{\textbf{Prijava}}	 \\ \hline[3pt]
					\SetCell{LightGreen}idRad & SERIAL	&  	jedinstveni identifikator rada \\ \hline
					\SetCell{LightGreen}idKonf	& SERIAL &   	jedinstveni identifikator konferencije\\ \hline 
